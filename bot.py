...
# Yandex endpoints (только для эмбеддингов)
...
def claude_answer(system: str, user: str) -> str:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        answer = claude_answer(SYSTEM_PROMPT, user_msg)
        ...
        await update.message.reply_text("⚠️ Ошибка Claude API. Попробуйте через минуту.")
    except aiohttp.ClientResponseError as e:
        log.error(f"Yandex API error: {e.status} {e.message}")
        await update.message.reply_text("⚠️ Ошибка поиска. Попробуйте через минуту.")
    except Exception as e:
