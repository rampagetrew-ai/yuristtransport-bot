...
def get_dialog_history(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> list[dict]:
    dialogs = context.bot_data.setdefault("dialogs", {})
...
def add_to_dialog(context: ContextTypes.DEFAULT_TYPE, chat_id: int, role: str, text: str):
    ...
    # Храним только последние N пар (каждая пара = 2 записи)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    asyncio.create_task(record_question(user.id, user.username or str(user.id), query))
